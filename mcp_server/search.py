...
def search_chunks(
    ...
    for idx, chunk in enumerate(all_chunks):
        ...
        if chunk_embedding is None or not isinstance(chunk_embedding, np.ndarray):
            # print(f"Warning: Chunk {idx} in {chunk['filename']} missing embedding, skipping.")
        ...
        try:
            ...
            similarity = dot_score(query_embedding, chunk_embedding)[0][0].item() # Get Python float
        except Exception as e:
            # Use stderr for errors
            import sys
        ...
        # Store results with scores (similarity score is now the relevance score)
        results_with_scores.append(
            {
                "filename": chunk["filename"],
                "heading": chunk["heading"],
                "content": chunk["content"], # Return original content
                "score": similarity, # Use cosine similarity as the score
